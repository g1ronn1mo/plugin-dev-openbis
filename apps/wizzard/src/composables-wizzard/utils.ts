...
export function createTableEntries(
  ...
  entetyConditionCombinations = entetyConditionCombinations.map(
    (combination) => {
      // Concatenate condition values to form secondaryName
      const concatName = combination
        .map((condition) => Object.values(condition)[0])
        .join(' ; ');
...
function generateConditionCombinations(tableVariables: TableVariable[]) {
  ...
  for (const item of tableVariables) {
    ...
    for (const condition of item.conditions) {
      ...
      if (item.continous === true) conditions.push(`${condition} ${item.unit}`);
...
export function crossProductSamples(
  parents: combinedVariable[],
  childs: combinedVariable[]
) {
