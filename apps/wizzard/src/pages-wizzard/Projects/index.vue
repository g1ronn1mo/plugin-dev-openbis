...
function updateEntety() {
  ...
  if (!isEqual) {
    entetyConditionsResult.value = createTableEntries(
      entetyVariables.value,
      SAMPLE_TYPE
    );
    tempEntetyVariables.value = JSON.stringify(entetyVariables.value);
  }
...
function updateBiol() {
  ...
  if (!isEqual) {
    sampleConditionsResult.value = createTableEntries(
      sampleVariables.value,
      SAMPLE_TYPE
    );
    entetyAndSampleResult.value = crossProductSamples(
      entetyConditionsResult.value,
      sampleConditionsResult.value
    );
    tempSampleVariables.value = JSON.stringify(sampleVariables.value);
  }
  return true;
...
function updateTech() {
  ...
  const isEqual =
    tempTechVariables.value == JSON.stringify(techVariables.value);
  if (!isEqual) {
    techConditionsResult.value = createTableEntries(
      techVariables.value,
      SAMPLE_TYPE
    );
    Result.value = crossProductSamples(
      entetyAndSampleResult.value,
      techConditionsResult.value
    );
...
async function onComplete() {
  ...
  await store.createProject(
    ...
    projectContext.value.space,
    projectContext.value.description
  );
}
</script>
