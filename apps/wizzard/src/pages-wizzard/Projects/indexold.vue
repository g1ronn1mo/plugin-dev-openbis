...
const variables = ref([{ title: 'genotype' }, { title: 'color' }]);
...
function checkProjectContext() {
  if (
    ...
    length(projectContext.value.description) < MINDESCLENGTH
  ) {
...
watch(tableVariables, (newValue, oldValue) => {
  for (let i = 0; i < newValue.length; i++) {
    ...
    if (!newValue[i].hasOwnProperty('condition')) newValue[i].condition = [];
  }
});
</script>
        ...
        <v-form>
          <FormWizard @on-complete="onComplete">
            <TabContent
              ...
              :before-change="checkProjectContext"
            >
              ...
              <div>
                <v-card>
                  <v-tabs v-model="tab" bg-color="primary">
                    <v-tab
                      v-for="(item, index) in tableVariables"
                      :key="index"
                      :value="item.title"
                    >
                      {{ item.title }}
                  ...
                  <v-card-text>
                    <v-window v-model="tab">
                      ...
                      >
                        ...
                        <div
                          v-if="
                            item.title === 'species' && variablespeciesNumber
                          "
                        >
                          <div
                            v-for="n in Number(variablespeciesNumber)"
                            :key="n"
                          >
                            Species {{ n }}
