...
async function loadVersions(wf_UUID: string) {
  try {
    ...
    const response = await fetch(`${worfklowApiRoute}/${wf_UUID}/versions`);
    versions.value = await response.json();
    if (versions.value.length === 1) setSelectedVersion(versions.value[0]);
  } catch (error) {
...
<template>
  ...
  <v-container>
    ...
    />
    <div v-else-if="versions.length === 1">Version: {{ versions[0].name }}</div>
